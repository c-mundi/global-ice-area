...
# plot image
img_file = '/Users/mundi/Downloads/snapshot-2011-04-16T00_00_00Z.tif' # nasa worldview file
...
print('--> see taylor5_revised.py') # updated
...
#%%% bar plot fxn - october 2000s

def seaice_lines10(years, census_path, area_path, si_path, AREA_IND, sample_size=10, p_thresh=984):
    ice_lims = [20,80]
    ...
    for year in years:
        ...
        census_file = census_path+'census10_'+str(year)+'.csv'
        [startdate, enddate] = fx.readCensus(census_file, convertDT=True)[0]
        pressure = fx.readCensus(census_file, convertDT=True)[-1]
        ...
        try:
            ds = xr.open_dataset(si_path + str(year) + '_seaice10' + '.nc')
        except:
            print('- skip: '+si_path + str(year) + '_seaice10' + '.nc')
            continue
...
#%%% bar plot with 90s, 00s

print(); print('*** global bars: new decades ***')
'''check y=1, spacing'''

decades1 = [np.arange(1982, 1992), np.arange(1990,2000), np.arange(2000,2010),np.arange(2010,2020)]
shortname = ['\'80s', '\'90s', '\'00s', '\'10s']
...
#### set up
fig2, axes2 = plt.subplots(2,1, figsize=(14,12), sharex=True, sharey=True)
for i, ax2 in enumerate(axes2.flatten()):
    ax2.axhline(0, ls='-', color='k', lw=1)
    ax2.set_xlim(-0.5,14.25)
    ax2.set_xticks(list(np.arange(XSPACING[1], 12+XSPACING[1]))+[13.175])
...
# get hatch patches for decade legend
import matplotlib.patches as mpatches
patches = []
for yi, years in enumerate(decades1[:-1]):
    patches.append(mpatches.Patch( facecolor='gray', edgecolor='k', alpha=0.5, hatch=HATCH[yi],
                           label=str(years[0])+'-'+str(years[-1])))
patches.append(mpatches.Patch( facecolor='gray', edgecolor='k', alpha=0.5,
                       label=str(decades1[-1][0])+'-'+str(decades1[-1][-1])))
    
ax2.legend(handles = patches, loc='lower left',
           fontsize=fs-1, handletextpad=0.5, handlelength=2)
...
for li, loc in enumerate(['Arctic','Antarctic']):
    ...
    era_values = {}
    min_values = {}; max_values = {}
    for yi, years in enumerate(decades1):
        yr_title = str(years[0])+'-'+str(years[-1])
        
        if years[0]==2010: trial = trials
        else: trial = {'original': trials['original']}
        ...
        for k, name in enumerate(list(trial.keys())):
            ...
            if years[0] in [2010, 1982]:
                mean_lines, lines, start_day, end_day, si_changes, clim_changes = \
                    seaice_lines(years, attr[0]+'census/', attr[0]+'area/', attr[1], 
                                 AREA_IND=attr[2], p_thresh=attr[3][li])
            elif years[0] in [1990, 2000]:
                path3 = path1 + 'intermediate_decades/'
                mean_lines, lines, start_day, end_day, si_changes, clim_changes = \
                    seaice_lines(years, path3+'census/', path3+'area/', path3+'seaice/', 
                                 AREA_IND=attr[2], p_thresh=attr[3][li])
                    
                # get summer decade dates
                if li==0:
                    path9 = '/Users/mundi/Desktop/FINAL/decades/'
                
                    mean_lines, lines6789, start_day, end_day, si_changes6789, clim_changes6789 = \
                        seaice_lines(years, path9+'census/', path9+'areas/', path9, 
                                     AREA_IND=0, p_thresh=984)
                        
                    for mo in [6,7,8,9]:
                        si_changes[mo] = si_changes6789[mo]
                        clim_changes[mo] = clim_changes6789[mo]
                        lines[mo] = lines6789[mo]
                        
                    path10 = '/Users/mundi/Desktop/month-hemi/nh_data/intermediate_decades/'
                    if years[0] == 2000:
                        mean_lines, lines10, start_day, end_day, si_changes10, clim_changes10 = \
                            seaice_lines10(years, path3+'census/', path3+'area10/', path3+'seaice/', 
                                         AREA_IND=0, p_thresh=984)
                            
                        si_changes[10] = si_changes10[10]
                        clim_changes[10] = clim_changes10[10]
                        lines[10] = lines10[10]
                    
            else: 
                raise ValueError('data for: '+yr_title)
            ...
            for mi, mm in enumerate(months):
                ...
                if name == 'original':
                    if years[0]==2010:
                        ...
                        axes2[0].bar(xvals[-1]+(WIDTH*(yi+1)), diffs[-1], width=WIDTH,
                                facecolor=hemi_colors[li], alpha=0.5, 
                                edgecolor=hemi_colors[li], hatch=HATCH[yi], lw=2)
        ...
        if years[0]==2010:
            ...
            axes2[0].bar(xvals[-1]+(WIDTH*(yi+1))+1.33+li, sums[-1], width=XSPACING[1]-XSPACING[0],
                    facecolor=hemi_colors[li], alpha=0.5, edgecolor=hemi_colors[li], hatch=HATCH[yi], lw=2)
        ...
        if vi==0:
            ...
            for mi, sum1 in enumerate(sums[0][0]):
                ...
                if years[0]==2010:
                    ...
                    axes2[1].bar(xvals[-1]+(WIDTH*(yi+1)), sum1[-1], width=WIDTH,
                            facecolor=ecs[-1], alpha=0.5, edgecolor=ecs[-1], hatch=HATCH[yi],lw=2)
            ...
            if years[0]==2010:
                ...
                axes2[1].bar(xvals[-1]+(WIDTH*(yi+1))+1.33+0.5, np.nansum(sums[0][0], axis=0)[-1], width=WIDTH,
                        facecolor='k', alpha=0.5, edgecolor='k', hatch=HATCH[yi], lw=2)
            
            print('total', years)
            total_number = np.nansum(sums[0][0], axis=0)[-1]
            print(total_number)
            axes2[1].text(12.5, -6-(4*yi), shortname[yi]+': '+str(round(total_number, 1)), fontsize=fs-1)
            
            axes2[1].axvline(1+xvals[0], ls='--', color='k', lw=0.85)
            
        # add spread bars!
        elif vi==1 and years[0]==2010:
            val0_min = val0
            val1_min = val1
        elif vi==2 and years[0]==2010:
            for mi in np.arange(0,12):
                xvals = mi+np.array(XSPACING)
...
for li, loc in enumerate(['Arctic','Antarctic']):
    ...
    for yi, years in enumerate(decades1):
        ...
        if years[0] in [2010, 1982]:
            mean_lines, lines, start_day, end_day, si_changes, clim_changes = \
                seaice_lines(years, path1+'census/', path1+'area/', path1+'seaice/', 
                             AREA_IND=0, p_thresh=[984,957][li])
        elif years[0] in [1990, 2000]:
            path3 = path1 + 'intermediate_decades/'
            mean_lines, lines, start_day, end_day, si_changes, clim_changes = \
                seaice_lines(years, path3+'census/', path3+'area/', path3+'seaice/', 
        ...
        counts = []
        for mi, mm in enumerate(months):
            counts.append(len(lines[mm]))
            
        axes[li].plot(months, counts, label = yr_title)
