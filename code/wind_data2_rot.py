...
LOC = 1 #0=arctic,1=antarctic

if LOC==0: ref_vec = (-1,1)
elif LOC==1: ref_vec = (-1,-1)
...
years = list(np.arange(2010,2020)) + list(np.arange(1982,1992))

time_pairs = []
path1 = root_path+ data_path[LOC]
for year in years:
    for month in np.arange(1,12+1):
       try: np.load(savepath+ savedir + ncnameadd+str(year)+'-'+str(month) +'.npy')
...
def calc_wind_timeseries(storm_event):
    ...
    if all_contours == []: print('empty contours'); return [np.nan]*len(analysis_range)
    ...
    def calc_winds(dt):
        ...
        # project new rotated frame!
        values = []
        for ux, vx in zip(u_wind.flatten(), v_wind.flatten()):
            values.append( np.dot((ux,vx), ref_vec) / np.linalg.norm(ref_vec) )
        thiswind = np.array(values).reshape(np.shape(u_wind))
        
        # wind timeseries daily value
        ...
        return np.nanmean(total_wind_miz)
...
while len(time_pairs)>0:
                   
    print('TIME_PAIRS:', len(time_pairs))
    
    for ti, pair in enumerate(time_pairs):
        year = pair[0]
        month = pair[1]
        ...
        try: # check if already exists?
            np.load(savepath+ savedir + ncnameadd+str(year)+'-'+str(month) +'.npy')
            continue
        ...
        try:
            ...
            name = ncnameadd+str(year)+'-'+str(month)
            ...
            @timethis
            def get_storm_winds(storm_ranges):
                wind_series = []
                for storm_range in storm_ranges:
                    wind_series.append( calc_wind_timeseries(storm_range) )
                return wind_series
            
            wind_series = get_storm_winds(storm_ranges)
                
            ## SAVE
            np.save(savepath + savedir + name +'.npy', wind_series)
            print('SAVED: '+ savedir + name +'.npy')
            # time_pairs.pop(ti) # remove this one
    ...
    for pair in time_pairs:
        ...
        try: np.load(savepath+ savedir + ncnameadd+str(year)+'-'+str(month) +'.npy')
        except (FileNotFoundError, EOFError): new_tp.append((year,month))
    time_pairs = new_tp
    
    # # check remianing time pairs after loop
    # time_pairs = []
    # path1 = root_path+ data_path[LOC]
    # for year in years:
    #     for month in np.arange(1,12+1):
    #        try: np.load(savedir+ savedir + ncnameadd+str(year)+'-'+str(month) +'.npy')
    #        except FileNotFoundError: 
    #            time_pairs.append((year,month))
