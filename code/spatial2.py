...
gs = GridSpec(nrows=nrows, ncols=ncols+2, 
              width_ratios=[1]*ncols + [0.55,0.55])
...
for ax, mname in zip(axes.flatten()[0:len(months)], mnames):
    ax.set_title(mname+'\n', fontsize=fontsize+2)
...
for loc_ind, loc in enumerate(['Arctic','Antarctic']):
    ...
    for yx, years in enumerate(decades):
        ...
        for mm, month_group in enumerate(months):
            ...
            for month in month_group:
                ...
                month_data.append(plot_var)
                
            plot_var = np.nansum(month_data, axis=0)
...
if plot_total:
    ...
    for loc_ind, loc in enumerate(['Arctic','Antarctic']):
        ...
        for yx, years in enumerate(decades):
            total_sum1 = []
            for mm, month in enumerate([mo for mg in months for mo in mg]):
                savename = 'plot_difference_'+str(month)+'_'+str(years[0])+'_'+str(years[-1])+'.npy'
        ...
        line = plt.Line2D((.5725,.5725),(.1,.875), color="k", linewidth=1.5)
        fig.add_artist(line)    
        print('*************')
