...
plt.plot();plt.title('Monthly normalized lines for each hemisphere, decade'+'\n'+
                     'Storm Area Comparison')
...
for AREA_IND in [1,2]:
    for loc_ind, loc in enumerate(hemi_names):
        ...
        for era, years in enumerate(decades):
            ...
            #### data plotting
            mean_line_og = fx.indiv_lines(years, path1+'census/', path1+'area/', path1+'seaice/')[0]
                
            mean_lines, lines, start_day, end_day, si_changes, clim_changes = \
            seaice_lines(years, path1+'census/', path1+'area/', path1+'sensitivity_seaice/', AREA_IND)
...
for AREA_IND in [1,2]:
    ...
    for li, loc in enumerate(['arctic','aa']):
        for yi, years in enumerate(decades):
            ...
            new_lines = seaice_lines(years, path1+'census/', path1+'area/', path1+'sensitivity_seaice/', AREA_IND)[0]
    ...
    for loc_ind, loc in enumerate(hemi_names):
        ...
        for era, years in enumerate(decades):
            ...
            for mi, month in enumerate(months):
                ...
                #### statistics
                mask = ~np.isnan(values_og) & ~np.isnan(values)
                t, pt = ttest_ind(values_og[mask], values[mask])
                ks, pks = ks_2samp(values_og[mask], values[mask])
                mw, pmw = mannwhitneyu(values_og[mask], values[mask])
...
for loc_ind, loc in enumerate(hemi_names):
    path1= root_paths[loc_ind]
    out_path = path1+'sensitivity_seaice/'
    print(loc)
    ...
    for era, years in enumerate(decades):
        ...
        for year in years:
           ...
           try: # open saved data
               ...
               census_path = path1+'census/'
               ncpath_area = path1+'area/'
               ncadd = '_areas'
               
               ncpath = path1+'sensitivity_seaice/'
               ncadd='_500kmradius' 
               
               #### get location of storm minimum pressure
               import pickle 
               file = open(path1+'all_storms/'+str(year) + '_all_storms.pkl', 'rb')
...
for li, loc in enumerate(['Arctic','Antarctic']):
    ...
    for yi, years in enumerate(decades):
        ...
        mean_lines1, lines1, start_day1, end_day1, si_changes1, clim_changes1 = \
            seaice_lines(years, path1+'census/', path1+'area/', path1+'sensitivity_seaice/', 1)
        mean_lines2, lines2, start_day2, end_day2, si_changes2, clim_changes2 = \
            seaice_lines(years, path1+'census/', path1+'area/', path1+'sensitivity_seaice/', 2)
...
#%% --
#%% compare pressure threshold storm counts
root1 = '/Users/mundi/Desktop/month-hemi/'
original_paths = [root1+'nh_data/', root1+'sh_data/']
sens_paths = [path+'sensitivity/census/' for path in original_paths]

for loc_ind, loc in enumerate(hemi_names):
    if loc_ind==0: continue
    print(loc)
    census_path = original_paths[loc_ind]+'census/'
    new_path = sens_paths[loc_ind]
    for year in np.arange(2010,2020): 
        print('*',year,'*')
        census_file1 = census_path+'census_'+str(year)+'.csv'
        [startdate1, enddate1] = fx.readCensus(census_file1, convertDT=True)[0]
        
        census_file2 = new_path+'census_'+str(year)+'.csv'
        [startdate2, enddate2] = fx.readCensus(census_file2, convertDT=True)[0]

        storms1, storms2 = {},{}
        for mm in months:
            storms1[mm] = [s for s in startdate1 if s.month==mm]
            storms2[mm] = [s for s in startdate2 if s.month==mm]
            
            if len(storms2[mm]) < len(storms1[mm]):
                print(str(mm)+':', len(storms1[mm]), len(storms2[mm]))
...
for loc_ind, loc in enumerate(hemi_names):
    if loc_ind==1: continue
    print(loc)
    census_path = original_paths[loc_ind]+'census/'
    new_path = sens_paths[loc_ind]
    
    storms1 = {mm:[] for mm in months}
    storms2 = {mm:[] for mm in months}
    for year in np.arange(2010,2020): 
        census_file1 = census_path+'census_'+str(year)+'.csv'
        [startdate1, enddate1] = fx.readCensus(census_file1, convertDT=True)[0]
    ...
    for mm in months:
        print(str(mm)+':', len(storms1[mm]), len(storms2[mm]), end='')
        if len(storms2[mm]) < len(storms1[mm]): print('*', end='')
        print()
